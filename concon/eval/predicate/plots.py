...
class ComplexityMemory:
    ...
    def _build_epochs_df(self):
        ...
        for dp in self.datapoints:
            ...
            properties = str(dp["config"].get("properties"))
            hidden     = dp["config"].get("hidden_size")
            has_neg = not dp["config"].get("no_negation", False)
            has_conj = not dp["config"].get("no_conjunction", False)
            ...
            epoch_budget = dp["config"].get("epochs")
            if epoch_budget is None:
                epoch_budget = int(eval_df["epoch"].max()) + 1 if "epoch" in eval_df.columns else np.nan

            results.append({
                    'complexity': int(complexity),
                    'properties': properties,
                    'hidden_size': hidden,
                    'negation': "+neg" if has_neg else "-neg",
                    'conjunction': "+conj" if has_conj else "-conj",
                    'case': "n-n" if has_conj else "n",
                    'epochs_to_max_acc': best_epoch_acc,
                    'epochs_to_max_perf': best_epoch_perf,
                    'epoch_budget': epoch_budget,
                    'max_acc': float(max_acc),
                    'max_perf': float(max_perf) if max_perf is not None else np.nan,
                })
    ...
    def _negation_color(self):
        return {"+neg": "#1f77b4", "-neg": "#e07a2d"}

    def _hidden_palette(self, values):
        vals = sorted(pd.Series(list(values)).dropna().unique())
        if not len(vals):
            return {}
        cols = sns.color_palette("viridis", n_colors=len(vals))
        return {v: cols[i] for i, v in enumerate(vals)}

    def _props_tick_labels(self, df):
        xticks = sorted(df["complexity"].unique())
        labels = []
        for c in xticks:
            vals = df.loc[df["complexity"] == c, "properties"].dropna().astype(str).unique()
            labels.append(vals[0] if len(vals) else str(c))
        return xticks, labels

    def _extract_run_id(self, run_name, fallback=None):
        m = re.search(r"__run=(\d+)", str(run_name))
        if m:
            return int(m.group(1))
        return fallback

    def _prepare_neg_df(self, neg_df):
        if neg_df is None or len(neg_df) == 0:
            return None
        df = neg_df.copy()
        if "complexity" not in df.columns and "num_predicates_cfg" in df.columns:
            df["complexity"] = df["num_predicates_cfg"]
        if "properties" in df.columns:
            df["properties"] = df["properties"].astype(str)
        req = {"complexity", "hidden_size", "xor_score", "homogeneity", "precision", "recall", "status", "run_name"}
        if not req.issubset(set(df.columns)):
            print(f"[WARN] Negation df missing columns: {sorted(req - set(df.columns))}")
            return None
    ...
    def _build_topsim_df(self):
        ...
        for dp in self.datapoints:
            ...
            if "eval/perf" in eval_df.columns:
                ...
                row = eval_df.iloc[-1]
            run_name = dp.get("run_name", dp.get("folder_name", ""))
            rows.append({
                "run_name": str(run_name),
                "complexity": int(cfg.get("num_predicates")) if cfg.get("num_predicates") is not None else np.nan,
    ...
    def plot_message_compression(self, *, group_by=("negation",), out_dir="plots"):
        ...
        if tuple(group_by) == ("negation",):
            neg_cols = self._negation_color()
            color_map = {
                ("+neg",): neg_cols["+neg"],
                ("-neg",): neg_cols["-neg"],
            }
            for k in ordered_keys:
                color_map.setdefault(k, "#444444")
        elif tuple(group_by) == ("hidden_size",):
            hidden_vals = [k[0] for k in ordered_keys]
            hcols = self._hidden_palette(hidden_vals)
            color_map = {k: hcols.get(k[0], "#444444") for k in ordered_keys}
        else:
    ...
    def _plot_epochs_condition(self, df, *, title_tag, filename_base, out_dir="plots", y_limits=None):
        if df is None or df.empty:
            return
        out_dir = pathlib.Path(out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)

        # Aggregated over hidden sizes: show mean + median for acc/perf.
        agg = (
            df.groupby("complexity")
            .agg(
                properties=("properties", "first"),
                acc_mean=("epochs_to_max_acc", "mean"),
                acc_median=("epochs_to_max_acc", "median"),
                acc_min=("epochs_to_max_acc", "min"),
                acc_max=("epochs_to_max_acc", "max"),
                perf_mean=("epochs_to_max_perf", "mean"),
                perf_median=("epochs_to_max_perf", "median"),
                perf_min=("epochs_to_max_perf", "min"),
                perf_max=("epochs_to_max_perf", "max"),
                max_perf_mean=("max_perf", "mean"),
            )
            .reset_index()
        )
        fig = plt.figure(figsize=(10, 6))
        plt.fill_between(agg["complexity"], agg["acc_min"], agg["acc_max"], alpha=0.15)
        plt.fill_between(agg["complexity"], agg["perf_min"], agg["perf_max"], alpha=0.12)
        plt.plot(agg["complexity"], agg["acc_mean"], marker="o", label="acc mean")
        plt.plot(agg["complexity"], agg["acc_median"], marker="o", linestyle=":", label="acc median")
        plt.plot(agg["complexity"], agg["perf_mean"], marker="s", label="perf mean")
        plt.plot(agg["complexity"], agg["perf_median"], marker="s", linestyle=":", label="perf median")
        for _, r in agg.iterrows():
            if pd.notna(r["perf_mean"]) and pd.notna(r["max_perf_mean"]):
                plt.text(r["complexity"], r["perf_mean"], f"{r['max_perf_mean']:.2f}", fontsize=8, ha="left", va="bottom")
        xticks, labels = self._props_tick_labels(agg)
        plt.xscale("log", base=2)
        plt.xticks(xticks, labels, rotation=45, ha="right")
        plt.title(f"epochs to max acc/perf by complexity ({title_tag}, avg hidden)")
        plt.xlabel("num_predicates (ticks), labels=properties")
        plt.ylabel("epochs")
        ...
        plt.legend()
        fig.tight_layout()
        fig.savefig(out_dir / f"{filename_base}_avg_hidden_{self.name}.png", dpi=150)

        # Stratified by hidden size.
        hcols = self._hidden_palette(df["hidden_size"])
        fig = plt.figure(figsize=(10, 6))
        for h, grp in df.groupby("hidden_size"):
            stats = (
                grp.groupby("complexity")
                .agg(
                    properties=("properties", "first"),
                    acc_mean=("epochs_to_max_acc", "mean"),
                    acc_min=("epochs_to_max_acc", "min"),
            ...
            col = hcols.get(h)
            plt.fill_between(stats["complexity"], stats["acc_min"], stats["acc_max"], alpha=0.12, color=col)
            plt.fill_between(stats["complexity"], stats["perf_min"], stats["perf_max"], alpha=0.10, color=col)
            plt.plot(stats["complexity"], stats["acc_mean"], marker="o", color=col, label=f"h{h} acc mean")
            plt.plot(stats["complexity"], stats["perf_mean"], marker="s", linestyle="--", color=col, label=f"h{h} perf mean")
            for _, r in stats.iterrows():
                if pd.notna(r["perf_mean"]) and pd.notna(r["max_perf_mean"]):
                    plt.text(r["complexity"], r["perf_mean"], f"{r['max_perf_mean']:.2f}", fontsize=7, ha="left", va="bottom", color=col)
        xticks, labels = self._props_tick_labels(df)
        plt.xscale("log", base=2)
        plt.xticks(xticks, labels, rotation=45, ha="right")
        plt.title(f"epochs to max acc/perf by complexity ({title_tag}, by hidden)")
        plt.xlabel("num_predicates (ticks), labels=properties")
        plt.ylabel("epochs")
        if y_limits is not None:
            plt.ylim(*y_limits)
        ...
        plt.legend(ncol=2)
        fig.tight_layout()
        fig.savefig(out_dir / f"{filename_base}_by_hidden_{self.name}.png", dpi=150)

    def plot_negation_metrics_by_complexity(self, neg_df, *, out_dir="plots"):
        df = self._prepare_neg_df(neg_df)
        if df is None:
            return

        out_dir = pathlib.Path(out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        metric_cols = [("xor_score", "xor"), ("homogeneity", "homogeneity"), ("f1_pr", "f1(pr)")]
        metric_colors = {"xor_score": "#1f77b4", "homogeneity": "#2ca02c", "f1_pr": "#d62728"}
        ...
        plt.xscale("log", base=2)
        plt.xticks(xticks, labels, rotation=45, ha="right")
        plt.title("negation metrics by complexity (+neg)")
        plt.xlabel("num_predicates (ticks), labels=properties")
        plt.ylabel("score")
        plt.grid(True, alpha=0.3)
        plt.legend()
        ...
        for ax, (col, label) in zip(axes, metric_cols):
            for h, grp in df.groupby("hidden_size"):
                stats = grp.groupby("complexity")[col].agg(mean="mean", median="median", lo="min", hi="max").reset_index()
                c = hcols.get(h)
                ax.fill_between(stats["complexity"], stats["lo"], stats["hi"], alpha=0.12, color=c)
                ax.plot(stats["complexity"], stats["mean"], marker="o", color=c, label=f"h{h} mean")
                ax.plot(stats["complexity"], stats["median"], marker="o", linestyle=":", color=c, alpha=0.8)
            ax.set_title(label)
        ...
        axes[1].set_xlabel("num_predicates (ticks), labels=properties")
        handles, labels_ = axes[0].get_legend_handles_labels()
        fig.suptitle("negation metrics by complexity (+neg), stratified by hidden size", y=0.99)
        fig.legend(
            handles,
            labels_,
            loc="upper center",
            bbox_to_anchor=(0.5, 0.95),
            ncol=min(4, max(1, len(labels_))),
            frameon=False,
        )
        fig.tight_layout(rect=[0, 0, 1, 0.88])
        fig.savefig(out_dir / f"negation_metrics_by_complexity_by_hidden_{self.name}.png", dpi=150)

    def plot_negation_profiles_comparison(self, neg_df_fast, neg_df_slow, *, out_dir="plots"):
        fast = self._prepare_neg_df(neg_df_fast)
        slow = self._prepare_neg_df(neg_df_slow)
        ...
        union_df = pd.concat([fast[["complexity", "properties"]], slow[["complexity", "properties"]]], ignore_index=True)

        fig, axes = plt.subplots(1, 3, figsize=(17, 5), sharey=True)
        for ax, (col, label) in zip(axes, metric_cols):
            for profile_name, (df, color) in profile_cfg.items():
                stats = df.groupby("complexity")[col].agg(mean="mean", median="median", lo="min", hi="max").reset_index()
                ax.fill_between(stats["complexity"], stats["lo"], stats["hi"], alpha=0.10, color=color)
                ax.plot(stats["complexity"], stats["mean"], marker="o", color=color, label=f"{profile_name} mean")
                ax.plot(stats["complexity"], stats["median"], marker="o", linestyle=":", color=color, alpha=0.9, label=f"{profile_name} median")
            ax.set_title(label)
            ax.grid(True, alpha=0.3)
            ax.set_xscale("log", base=2)
            xticks, labels = self._props_tick_labels(union_df)
            ax.set_xticks(xticks)
            ax.set_xticklabels(labels, rotation=45, ha="right")
        axes[0].set_ylabel("score")
        axes[1].set_xlabel("num_predicates (ticks), labels=properties")
        handles, labels_ = axes[0].get_legend_handles_labels()
        fig.legend(handles, labels_, loc="upper center", bbox_to_anchor=(0.5, 0.97), ncol=4, frameon=False)
        fig.suptitle("negation metrics by complexity: fast vs slow")
        fig.tight_layout(rect=[0, 0, 1, 0.88])
        fig.savefig(out_dir / f"negation_metrics_profile_compare_{self.name}.png", dpi=150)
    ...
    def plot_negation_metric_slopes(self, neg_df, *, profile_tag="", out_dir="plots"):
        ...
        df = df.copy()
        df["run_id"] = [
            self._extract_run_id(rn, fallback=int(ri) if pd.notna(ri) else None)
            for rn, ri in zip(df["run_name"], df.get("run_idx", pd.Series([np.nan] * len(df))))
        ]
        slope_df = (
            df.groupby(["run_id", "complexity"])
            .agg(
                properties=("properties", "first"),
                xor_score=("xor_score", "mean"),
        ...
        for ax, (col, label) in zip(axes, metric_cols):
            ...
            ax.grid(True, alpha=0.3)
            ax.set_xscale("log", base=2)
            xticks, labels = self._props_tick_labels(slope_df)
            ax.set_xticks(xticks)
            ax.set_xticklabels(labels, rotation=45, ha="right")
        axes[0].set_ylabel("score")
        axes[1].set_xlabel("num_predicates (ticks), labels=properties")
        ...
        profile_text = f" ({profile_tag})" if profile_tag else ""
        fig.suptitle(f"negation metric slopes across complexity{profile_text}")
        fig.tight_layout(rect=[0, 0, 1, 0.88])
    ...
    def plot_topsim_vs_negation(self, neg_df, *, profile_tag="", out_dir="plots"):
        ...
        if df is None or topsim_df is None:
            return
        merged = df.merge(
            topsim_df[["run_name", "topsim_intensional_levenshtein"]],
            on="run_name",
            how="inner",
        )
        merged = merged.dropna(subset=["topsim_intensional_levenshtein"])
        if merged.empty:
            return
        ...
        profile_text = f" ({profile_tag})" if profile_tag else ""
        fig.suptitle(f"topsim vs negation structure{profile_text}")
    ...
    def plot_topsim_interaction_xor(self, neg_df, *, profile_tag="", out_dir="plots"):
        df = self._prepare_neg_df(neg_df)
        topsim_df = self._build_topsim_df()
        if df is None or topsim_df is None:
            return
        ...
        x = merged["topsim_intensional_levenshtein"].to_numpy(dtype=float)
        y = merged["xor_score"].to_numpy(dtype=float)
        ...
        plt.ylabel("xor score")
        plt.title("topsim x complexity interaction for xor")
        plt.grid(True, alpha=0.3)
        plt.legend()
        ...
        profile_text = f" ({profile_tag})" if profile_tag else ""
        if profile_text:
            plt.title(f"topsim x complexity interaction for xor{profile_text}")
        fig.tight_layout()
        tag = str(profile_tag) if profile_tag else "default"
        fig.savefig(out_dir / f"topsim_xor_interaction_{tag}_{self.name}.png", dpi=150)

    def plot_candidate_count_vs_score_gain(self, neg_df_fast, neg_df_slow, *, out_dir="plots"):
        fast = self._prepare_neg_df(neg_df_fast)
        slow = self._prepare_neg_df(neg_df_slow)
        if fast is None or slow is None:
            return
        if "n_candidates_total" not in fast.columns or "n_candidates_total" not in slow.columns:
            print("[WARN] Candidate-count gain plot skipped; n_candidates_total not found.")
            return
        key_cols = ["run_name", "complexity", "hidden_size", "properties"]
        available_keys = [c for c in key_cols if c in fast.columns and c in slow.columns]
        merged = fast.merge(
            slow,
            on=available_keys,
        ...
        if merged.empty:
            return
        merged["delta_xor"] = merged["xor_score_slow"] - merged["xor_score_fast"]
        merged["delta_homogeneity"] = merged["homogeneity_slow"] - merged["homogeneity_fast"]
        merged["delta_f1_pr"] = merged["f1_pr_slow"] - merged["f1_pr_fast"]

        out_dir = pathlib.Path(out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        metric_cols = [("delta_xor", "Δxor"), ("delta_homogeneity", "Δhomogeneity"), ("delta_f1_pr", "Δf1(pr)")]
        cvals = np.log2(np.maximum(merged["complexity"].astype(float), 1.0))
        x = merged["n_candidates_total_slow"].astype(float).to_numpy()
        fig, axes = plt.subplots(1, 3, figsize=(17, 5), sharex=True)
        scatter_obj = None
        for ax, (col, label) in zip(axes, metric_cols):
            y = merged[col].astype(float).to_numpy()
            scatter_obj = ax.scatter(x, y, c=cvals, cmap="viridis", alpha=0.85, edgecolors="none")
            mask = np.isfinite(x) & np.isfinite(y)
            if mask.sum() >= 2 and np.ptp(x[mask]) > 1e-12:
                a, b = np.polyfit(x[mask], y[mask], deg=1)
                xg = np.linspace(x[mask].min(), x[mask].max(), 100)
            ...
            ax.set_title(label)
            ax.grid(True, alpha=0.3)
            ax.set_xlabel("slow n_candidates_total")
        axes[0].set_ylabel("slow - fast")
        if scatter_obj is not None:
            cax = fig.add_axes([0.92, 0.16, 0.015, 0.68])
            cbar = fig.colorbar(scatter_obj, cax=cax)
            cbar.set_label("log2(complexity)")
        fig.suptitle("candidate count vs score gain (slow vs fast)")
        fig.subplots_adjust(left=0.06, right=0.89, bottom=0.14, top=0.86, wspace=0.30)
        fig.savefig(out_dir / f"candidate_count_vs_score_gain_{self.name}.png", dpi=150)

    def plot_requested_suite(self, *, neg_df=None, out_dir="plots"):
        epochs_df = self._build_epochs_df()
        if epochs_df is not None and not epochs_df.empty:
            y_limits = (0.0, 50.0)

            subsets = [
                ("n+neg", epochs_df[(epochs_df["case"] == "n") & (epochs_df["negation"] == "+neg")], "epochs_n_posneg"),
                ("n-neg", epochs_df[(epochs_df["case"] == "n") & (epochs_df["negation"] == "-neg")], "epochs_n_noneg"),
                ("n-n", epochs_df[epochs_df["case"] == "n-n"], "epochs_n_minus_n"),
            ]
            for title_tag, sub_df, filebase in subsets:
                self._plot_epochs_condition(
                    sub_df,
                    title_tag=title_tag,
            ...
            # Message compression:
            # - n: by negation (no hidden split)
            # - n-n: by hidden size
            dps_n = [dp for dp in self.datapoints if dp.get("config", {}).get("no_conjunction", True)]
            dps_nn = [dp for dp in self.datapoints if not dp.get("config", {}).get("no_conjunction", True)]
            if dps_n:
                ComplexityMemory(dps_n, name=f"{self.name}_n").plot_message_compression(group_by=("negation",), out_dir=out_dir)
                hidden_vals = sorted({dp.get("config", {}).get("hidden_size") for dp in dps_n})
                for h in hidden_vals:
                    sub = [dp for dp in dps_n if dp.get("config", {}).get("hidden_size") == h]
                    if sub:
                        ComplexityMemory(sub, name=f"{self.name}_n_h{h}").plot_message_compression(group_by=("negation",), out_dir=out_dir)
            if dps_nn:
                ComplexityMemory(dps_nn, name=f"{self.name}_nn").plot_message_compression(group_by=("hidden_size",), out_dir=out_dir)
                hidden_vals = sorted({dp.get("config", {}).get("hidden_size") for dp in dps_nn})
                for h in hidden_vals:
                    sub = [dp for dp in dps_nn if dp.get("config", {}).get("hidden_size") == h]
                    if sub:
                        ComplexityMemory(sub, name=f"{self.name}_nn_h{h}").plot_message_compression(group_by=("hidden_size",), out_dir=out_dir)

        if neg_df is not None and len(neg_df):
            self.plot_negation_metrics_by_complexity(neg_df, out_dir=out_dir)
