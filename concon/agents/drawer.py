...
class Drawer(Agent):
    ...
    def forward(self, message, length):
        ...
        encoding = encoding[:,:,None,None].view(-1, encoding.size(-1), 1, 1) # Because the deconvolution expects a 1 by 1 image with D channels. Shape: (TODO) TODO useful?
        image = self.image_decoder(encoding) # Shape: (TODO)
